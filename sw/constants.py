"""
Physical constants, reference grids, and lookup tables for RRTM_SW.

All values taken verbatim from the Fortran source (rrtm.f, setcoef.f).
Physical constants and reference grids are identical to LW.
...
# ── Tropopause pressure thresholds ────────────────────────────────────────────
# Layers with ln(p) > LAYTROP_PLOG_THRESHOLD use lower-atmosphere (KA) coefficients;
# layers at or above use upper-atmosphere (KB) coefficients.
LAYTROP_PLOG_THRESHOLD = 4.56   # ln(pressure in mb)

# Layers with ln(p) > LAYSWTCH_PLOG_THRESHOLD use lower-atmosphere coefficients
# for the layswtch/laylow distinction in SW taumol routines.
LAYSWTCH_PLOG_THRESHOLD = 6.62  # ln(pressure in mb)
